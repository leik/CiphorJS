...
var Eve = (function() {
	/**
	 * Eve object including events registry and simple functions to listen and
	 * fire event.
	 */
	var Eve = {
		__events_pool : {},
	...
	Eve.event.prototype = {
		 ...
		 */
		never : function(fn) {
			for ( var i in this._functions.prioritised) {
				if (this._functions.prioritised[i].fn == fn) {
					delete this._functions.prioritised[i];
				}
			}
			for ( var i in this._functions.unprioritised) {
